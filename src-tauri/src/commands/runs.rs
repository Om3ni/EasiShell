...
/// Kick off a run of `command`. Output streams back over `on_event`; the return value
/// is just the run id so the frontend can tag what it's watching.
#[tauri::command]
pub async fn start_run(
    ...
    run_manager::start(state.inner(), command, on_event).await
}
